...
# Check Git (per cloning del progetto se necessario)
try {
...
# Funzione per input con default
...
$Config = @{}
$Config.proxy_host = Read-ConfigValue "Proxy hostname" "100.108.112.117" $ExistingConfig.proxy_host
$Config.proxy_port = [int](Read-ConfigValue "Proxy port" "5432" $ExistingConfig.proxy_port)

Write-ColorOutput "`n🗄️  Database Configuration" $Colors.Cyan
...
Write-Host "🗄️  Database: $($Config.db_name) on $($Config.db_host):$($Config.db_port)"
Write-Host "👤 User: $($Config.db_user)"
Write-Host "🔐 SSL Mode: $($Config.ssl_mode)"
Write-Host "🌐 Tailscale: $($Config.tailscale_ip)"
Write-Host "⏱️  Timeout: $($Config.test_timeout)s"
...
if ($TcpSuccess) {
    Write-Success "TCP connection successful ($($ConnectTime.TotalMilliseconds.ToString('F2'))ms)"
} else {
...
if (Get-Command python -ErrorAction SilentlyContinue) {
    Write-Host "🗄️  Testing PostgreSQL..."
    ...
    Write-Host "⚠️  Python not available for PostgreSQL test" -ForegroundColor Yellow
}
...
if (Test-Path "manage.py") {
    ...
    Write-Host "⚠️  No Django project found in current directory" -ForegroundColor Yellow
...
Write-ColorOutput "3. 📊 Run comprehensive tests:" $Colors.Yellow
Write-Host "   python proxy_test_suite.py --config-file `"$ConfigFile`""
Write-Host ""

Write-ColorOutput "4. 🔒 Run security audit:" $Colors.Yellow
Write-Host "   python proxy_security_audit.py --config-file `"$ConfigFile`""
Write-Host ""

Write-ColorOutput "5. 🐍 Test Django connection:" $Colors.Yellow
Write-Host "   PowerShell $DjangoTestScript"
Write-Host ""

Write-ColorOutput "6. 🚀 Run database migration:" $Colors.Yellow
Write-Host "   python database_migration_utility.py --config-file `"$ConfigFile`" --sqlite-path db.sqlite3"
Write-Host ""
